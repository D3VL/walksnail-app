...
export const meta: MetaFunction = () => {
    return [
        ...
        { name: "description", content: "Archive of all Walksnail Avatar & Ascent firmware releases, including private beta releases." },
...
export default function () {
    ...
    const [releaseSystemFilter, setReleaseSystemFilter] = useState<"avatar" | "ascent">("avatar");

    return (
        <div className="container md:px-6">

            <h1 className="text-3xl font-bold my-6">All WalkSnail Avatar & Ascent Firmware Releases</h1>
            <p>
            ...
            <div className="flex flex-row flex-wrap gap-4 my-4">
                {/* <div className="mb-4">
                    <input type="radio" defaultChecked name="filter" id="filter_all" className="sr-only peer" onChange={(e) => e.target.checked && setReleaseSystemFilter("all")} checked={releaseSystemFilter === "all"} />
                    <label htmlFor="filter_all" className="cursor-pointer p-2 rounded-lg bg-gray-200 border-gray-300 border-2 peer-checked:border-accent" >All Firmware</label>
                </div> */}

                <div className="mb-4">
                    <input type="radio" name="filter" id="filter_avatar" className="sr-only peer" onChange={(e) => e.target.checked && setReleaseSystemFilter("avatar")} checked={releaseSystemFilter === "avatar"} />
                    <label htmlFor="filter_avatar" className="cursor-pointer p-2 rounded-lg bg-gray-200 border-gray-300 border-2 peer-checked:border-accent" >Avatar</label>
                </div>

                <div className="mb-4">
                    <input type="radio" name="filter" id="filter_ascent" className="sr-only peer" onChange={(e) => e.target.checked && setReleaseSystemFilter("ascent")} checked={releaseSystemFilter === "ascent"} />
                    <label htmlFor="filter_ascent" className="cursor-pointer p-2 rounded-lg bg-gray-200 border-gray-300 border-2 peer-checked:border-accent" >Ascent</label>
                </div>
            </div>
            ...
            <div className="flex flex-col  my-6">
                ...
                {releases.filter((release: any) => releaseSystemFilter === "all" || release.system.toLowerCase() === releaseSystemFilter.toLowerCase())
                            ...
                            <Fragment key={i}>
                                <section className="grid grid-cols-12 gap-4">
                                    ...
                                    <div className="col-span-12 lg:col-span-10 mb-8">
                                        <Link to={`/firmware/${release.system.toLowerCase()}/${release.tag}`}>
                                            <RoundedCard className="relative flex flex-col gap-1 justify-between">
                                                ...
                                                <small className="text-accent uppercase font-bold text-xs leading-3 mb-1">{release.system}</small>
